...
class PartialFourierModule:
    ...
    def __call__(connection_buffer: ismrmrd_tools.ConnectionBuffer,
        ...
        @author Jörn Huber
        """
        return PartialFourierModule.apply(connection_buffer,  book_keeper)
    ...
    def apply(connection_buffer: ismrmrd_tools.ConnectionBuffer,
        ...
        if connection_buffer.meas_data.pf_factor_pe1 != 1.0 or connection_buffer.meas_data.pf_factor_pe2 != 1.0:
            ...
            logging.info("gs-recon: Running POCS partial Fourier routine")
            ...
            recon_mat_size_x = connection_buffer.meas_data.recon_space.matrixSize.x
            recon_mat_size_y = connection_buffer.meas_data.recon_space.matrixSize.y
            recon_mat_size_z = connection_buffer.meas_data.recon_space.matrixSize.z
            encode_mat_size_y = connection_buffer.meas_data.encoded_space.matrixSize.y
            encode_mat_size_z = connection_buffer.meas_data.encoded_space.matrixSize.z

            connection_buffer.meas_data.data['NP_IS_IMAGING'] = np.zeros((recon_mat_size_x,
                                                                                recon_dims[1],
                                                                                recon_mat_size_y,
                                                                                recon_mat_size_z,
                                                                                recon_dims[4],
            ...
            if connection_buffer.meas_data('NP_IS_IMAGING', 'PE2') > 1:
                ...
                q_pe1 = max(3, int((connection_buffer.meas_data('NP_IS_IMAGING', 'PE1') -
                                    connection_buffer.meas_data('NP_IS_IMAGING', 'PE1') / 2) / 6))
                if q_pe1 % 2 == 0:
                    q_pe1 = q_pe1 + 1
                q_pe2 = max(3, int((connection_buffer.meas_data('NP_IS_IMAGING', 'PE2') -
                                    connection_buffer.meas_data('NP_IS_IMAGING', 'PE2') / 2) / 6))
                if q_pe2 % 2 == 0:
                    q_pe2 = q_pe2 + 1
                            ...
                            for i_con in range(0, connection_buffer.meas_data('NP_IS_IMAGING', 'CON')):
                                for i_slc in range(0, connection_buffer.meas_data('NP_IS_IMAGING', 'SLC')):
                                    for i_cha in range(0, connection_buffer.meas_data('NP_IS_IMAGING', 'CHA')):
                                        ...
                                        cha_pf_image_zero = np.zeros((recon_mat_size_x,
                                                                      recon_mat_size_y,
                                                                      recon_mat_size_z), dtype=complex)

                                        cha_pf_image_zero[:, 0:encode_mat_size_y, 0:encode_mat_size_z] = cha_pf_image

                                        cha_pf_image_filled = (
                                            parallel_tools.fill_partial_fourier_pocs_3D(cha_pf_image_zero, 5, q_pe1, q_pe2))
                                        ...
                                        connection_buffer.meas_data.data['NP_IS_IMAGING'][
                                            :, i_cha, :, :, i_slc, i_set, i_phase, i_con, i_rep, 0, 0] = cha_pf_image_filled
                            ...
                            for i_con in range(0, connection_buffer.meas_data('NP_IS_IMAGING', 'CON')):
                                for i_slc in range(0, connection_buffer.meas_data('NP_IS_IMAGING', 'SLC')):
                                    for i_cha in range(0, connection_buffer.meas_data('NP_IS_IMAGING', 'CHA')):
                                        ...
                                        cha_pf_zero = np.zeros((recon_mat_size_x, recon_mat_size_y), dtype=complex)
                                        cha_pf_zero[:, 0:encode_mat_size_y] = cha_pf
            ...
            book_keeper.recon_history += "_POCSPartialFourier"
